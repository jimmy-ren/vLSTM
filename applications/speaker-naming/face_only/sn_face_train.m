...
addpath data/speaker-naming/processed_training_data/val_face/
...
test_samples_base = test_samples;
...
for p = 1:100
    for m = 1:1
        load(strcat('data/speaker-naming/processed_training_data/train_face/', num2str(m)));
        labels = reshape(labels, size(labels,1), 1, size(labels,2));
        labels = repmat(labels, [1 config.max_time_steps 1]);
